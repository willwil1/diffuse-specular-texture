...
int main()
{
	...
	Shader ourShader("shader/vshader.vs", "shader/fshader.frag");
	Shader lightShader("shader/light_vshader.vs", "shader/light_fshader.frag");
...
/* //with texture
	...
	*/
	...
	int width, height;
	unsigned char* image = SOIL_load_image("texture/diffusemap.png", &width, &height, 0, SOIL_LOAD_RGB);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	// Load, create texture and generate mipmaps
	image = SOIL_load_image("texture/specularmap.png", &width, &height, 0, SOIL_LOAD_RGB);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, image);
	glGenerateMipmap(GL_TEXTURE_2D);
	SOIL_free_image_data(image);
...
//		glUniform3f(matDiffuseLoc, 1.0f, 0.5f, 0.31f);
		...
		// Draw container
		glBindVertexArray(VAO);
		for (GLuint i = 0; i < 10; i++) //can be 10
		{
			glm::mat4 model;
		...
		GLint l_modelLoc = glGetUniformLocation(lightShader.Program, "model");
		GLint l_viewLoc = glGetUniformLocation(lightShader.Program, "view");
		GLint l_projLoc = glGetUniformLocation(lightShader.Program, "projection");
		// Pass the matrices to the shader
		glUniformMatrix4fv(l_viewLoc, 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(l_projLoc, 1, GL_FALSE, glm::value_ptr(projection));
		glm::mat4  model;
